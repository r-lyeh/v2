...
EDITOR_FUNCTION(fullscreen, "(held(ALT)&down(ENTER))|down(F11)",{ record_stop(), window_fullscreen( window_has_fullscreen() ^ 1 ); } // close any recording before framebuffer resizing, which would corrupt video stream
...
void editor_frame( void (*game)(unsigned, float, double) ) {
    ...
    UI_MENU(14, \
        ...
        if(ingame) ui_enable(); \
        UI_MENU_ITEM(ICON_PL4Y, if(editor.t == 0) editor_key("eject"); editor_key(window_has_pause() ? "play" : "pause")) \
        UI_MENU_ITEM(ICON_SKIP, editor_key(window_has_pause() ? "frame" : "slomo")) \
        UI_MENU_ITEM(ICON_MDI_STOP, editor_key("stop")) \
        UI_MENU_ITEM(ICON_MDI_EJECT, editor_key("eject")) \
        ...
        if(ingame) ui_disable(); \
        UI_MENU_ITEM(ICON_MD_FOLDER_SPECIAL, editor_key("browser")) \
        UI_MENU_ITEM(ICON_MDI_FILE_TREE, editor_key("scene")) \
        UI_MENU_ITEM(ICON_MDI_SCRIPT_TEXT, editor_key("script")) \
        UI_MENU_ITEM(ICON_MDI_CHART_TIMELINE, editor_key("timeline")) \
        UI_MENU_ITEM(ICON_MDI_CONSOLE, editor_key("console")) \
        UI_MENU_ITEM(ICON_MDI_GRAPH, editor_key("nodes")) \
        if(ingame) ui_enable(); \
        UI_MENU_ITEM(window_has_maximize() ? ICON_MDI_WINDOW_MINIMIZE : ICON_MDI_WINDOW_MAXIMIZE, window_maximize(1 ^ window_has_maximize())) \
        UI_MENU_ITEM(ICON_MDI_CLOSE, editor_key("quit")) \
    );
