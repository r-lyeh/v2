...
int ui_editor() {
    ...
    UI_DETACHABLE_SECTION(va(UI_ICON(STORAGE) " Storage %s", xstats()),
        // @todo
    );
    UI_DETACHABLE_SECTION(UI_ICON(WIFI)/*SIGNAL_CELLULAR_ALT*/ " Network 0/0 KiB",
        // @todo
    ...
    UI_DETACHABLE_SECTION(UI_ICON(BUG_REPORT) " Bugs 0",
    ...
    UI_DETACHABLE_SECTION(UI_ICON(BOOKMARK) " Bookmarks",0); /* @todo */


    // E,C,S,W
    UI_DETACHABLE_SECTION(UI_ICON(ACCOUNT_TREE) " Scene",
    ...
    UI_DETACHABLE_SECTION(UI_ICON(ROCKET_LAUNCH) " AI",
        // @todo
    );
    UI_DETACHABLE_SECTION(UI_ICON(VOLUME_UP) " Audio",
        // ui_audio();
    );
    UI_DETACHABLE_SECTION(UI_ICON(PHOTO_CAMERA) " Camera",
        ui_camera( camera_get_active() );
    );
    UI_DETACHABLE_SECTION(UI_ICON(CAPTURE) " Display",0
//        // @todo: fps lock, fps target, aspect ratio, fullscreen
//        char *text = va("%s;%s;%s;%s",
//            window_has_fullscreen() ? FULLSCREEN_EXIT : FULLSCREEN,
//            PHOTO_CAMERA,
...
//        if( choice == 4 ) engine_send("key_pixdebug",0);
    ...
    UI_DETACHABLE_SECTION(UI_ICON(CONTENT_PASTE) " Scripts",
        // @todo
    );
    ...
    UI_DETACHABLE_SECTION(UI_ICON(SAVINGS) " Budgets",
    ...
    UI_DETACHABLE_SECTION(UI_ICON(POWER) " Power",0
...
//      if( choice == 2 ) engine_send("key_battery","1");
    ...
    UI_DETACHABLE_SECTION(UI_ICON(WATER) " Reflection",0
//        ui_reflect("*");
    );

    UI_DETACHABLE_SECTION(UI_ICON(EXTENSION) " Plugins",
        // @todo. include VCS
