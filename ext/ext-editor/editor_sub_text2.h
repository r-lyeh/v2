...
#define TEXTEDIT_ICON  UI_ICON(SCRIPT)
...
int subeditor_textedit(int submode) {
    ...
    do_once {
        ...
        lt_init(L, app_handle("window"), LT_DATAPATH, __argc, __argv, window_scale(), platform, pathexe);
    }
    ...
    if( subeditor_begin(submode, TEXTEDIT_TITLE, &open) ) {
        ...
        lt_mx = input(MOUSE_X) - bounds.Min.x;
        lt_my = input(MOUSE_Y) - bounds.Min.y;
        lt_wx = bounds.Min.x;
        lt_wy = bounds.Min.y;
