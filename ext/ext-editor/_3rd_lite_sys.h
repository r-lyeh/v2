...
#define lt_setwindowmode(m)  app_fullscreen(m == 2), (m < 2 && (window_maximize(m),1)) // 0:normal,1:maximized,2:fullscreen
#define lt_setwindowtitle(t) //app_settitle(t)
#define lt_haswindowfocus()  app_has_focus()
#define lt_setcursor(shape)  mouse_cursor(lt_events & (1<<31) ? 1/*auto*/ : shape+2/*specific*/) // 0:arrow,1:ibeam,2:sizeh,3:sizev,4:hand
...
int lt_poll_event(lua_State *L) { // init.lua > core.step() wakes on mousemoved || inputtext
    ...
    for( int i = 0, count = array_count(sdl3_events); i < count; ++i ) {
        ...
        default:
            ...
            rc += lt_emit_event(L, "mousepressed", "sddd", lt_button_name(e.button.button), lt_mx, lt_my, printi(1 + clicks));
