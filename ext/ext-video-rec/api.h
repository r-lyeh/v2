...
bool record_start(const char *outfile_mp4, unsigned flags) {
    ...
    const char *ffmpeg_exe = ifdef(win32, "ext\\ext-video-ffmpeg\\ffmpeg", "ext/ext-video-ffmpeg/ffmpeg")
...
//                  "-vf \"pad=ceil(iw/2)*2:ceil(ih/2)*2\" "
    ...
    if( !rec_ffmpeg ) {
        PRINTF("ext-video-ffmpeg not found: using mpeg-1 fallback (hint: consider installing ext-video-ffmpeg)\n");
