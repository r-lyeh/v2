 ...
 *
 * Latest revisions:
 *  1.04b rgbx -> bgrx channel swap && vertical image flip && userdef components (@r-lyeh)
...
#ifndef JO_MPEG_COMPONENTS //< @r-lyeh
    ...
    for (int vblock=0; vblock<(height+15)/16; vblock++) {
        for (int hblock=0; hblock<(width+15)/16; hblock++) {
            ...
            for (int i=0; i<256; ++i) {
                ...
                // const unsigned char *c = rgbx + y*width*4+x*4; // original
                // float r = c[0], g = c[1], b = c[2]; // original
                const unsigned char *c = rgbx + ((height-1)-y)*width*JO_MPEG_COMPONENTS+x*JO_MPEG_COMPONENTS; //< @r-lyeh: flipped Y
                float b = c[0], g = c[1], r = c[2]; //< @r-lyeh: rgbx -> bgrx channel swap
