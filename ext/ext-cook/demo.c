...
int main() {
    ...
    unsigned zlevel = optioni("--level,-l", 6);
    ...
    zip* z = zipfile ? zip_open(zipfile, "wb") : NULL;

    int count;
    char **list = file_list(option("--input,-i", "demos/**;embed/**;ext/**/embed/*"), &count);
    while( count-- ) {
        ...
        if( z ) {
            zip_append_mem(z, va("%s.%s", file_path(list[count]), file_name(list[count])), ""/*comment*/, cooked, size, zflags | zlevel);
        }
    }
