...
void camera_freefly(camera_t *cam, bool blocked) {
    ...
    if ( len3sq(wasdecq) ) cam->speed_buildup += (cam->speed * cam->accel * (2.0f * mult_speed + 1.0f) * window_delta());
    // if (!active) cam->speed_buildup = 1.0f;
    else cam->speed_buildup = 1.0f;
    camera_moveby(cam, scale3(wasdecq, window_delta() * 60 * cam->speed_buildup * (2.0f * mult_speed + 1.0f)));
    camera_fps(cam, mouse.x,mouse.y);
}
