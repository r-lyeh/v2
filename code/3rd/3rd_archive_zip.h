...
#ifndef MEMBOUNDS
#define MEMBOUNDS(...)     ((unsigned)0)
#endif

#ifndef MEMEXCESS
#define MEMEXCESS(...)     ((unsigned)0)
#endif
    ...
    static unsigned MEMBOUNDS(unsigned inlen, unsigned flags) {
        return ( flags > 10 ? mem_bounds : deflate_bounds )(inlen, flags);
    }
    static unsigned MEMEXCESS(unsigned flags) {
...
bool zip_append_file_timeinfo(zip *z, const char *entryname, const char *comment, FILE *in, unsigned compress_level, struct tm* timeinfo) {
    ...
    *e = zero;
    ...
    unsigned dataSize = e->header.uncompressedSize, compSize = MEMBOUNDS(e->header.uncompressedSize, compress_level);
