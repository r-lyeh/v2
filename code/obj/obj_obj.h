...
#define OBJ_HAS_SINGLE_PARENT 0

#if OBJ_HAS_SINGLE_PARENT
#define ifdef_OBJ_HAS_SINGLE_PARENT ifdef_1
#else
#define ifdef_OBJ_HAS_SINGLE_PARENT ifdef_0
#endif
...
#define OBJHEADER \
    struct { \
        ...
        ifdef(OBJ_HAS_SINGLE_PARENT,,array(struct obj*) objparents;) \
    };
...
#define obj_flag(o,oper,mask) ((o->objflags) oper (mask))
...
#define each_objchild(p,T,o) /*non-recursive*/ \
    (array(struct obj*)* children = obj_children(p); children; children = 0) \
        for(int _i = 1, _end = array_count(*children); _i < _end; ++_i) \
            for(T o = (T)((*children)[_i]); o ifdef(OBJ_HAS_SINGLE_PARENT, && (obj_parent(o) == p)); o = 0)
...
#else
API array(obj*)*obj_parents(const void *o);
API obj*        obj_detach1(void *c, void *p);
#endif
...
array(obj*)* obj_parents(const void *o) {
    array(obj*) *c = &((obj*)o)->objparents;
    if(!(*c)) array_push((*c), NULL); // default parenting: none. @todo: optimize & move this at construction time
    return c;
}

obj* obj_detach1(void *c, void* p) {
    if( p ) {
        uintptr_t id = obj_id(c);
...
obj* obj_detach(void *c) {
    for each_array(*obj_parents(c), obj*, p ) {
        obj_detach1(c, p);
    }
    return 0;
}
