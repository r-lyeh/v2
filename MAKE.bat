...
if not "%1"=="" (
        ...
        call make code\cook.c && cook

        where /q bundler.exe && (
        del .art.zip >nul 2>nul
        del filelist >nul 2>nul
        for /R demos %%i in (.*) do echo %%i >> filelist
        for /R ext %%i in (.*) do echo %%i >> filelist
        bundler a .art.zip @filelist -u lz4f
        del filelist >nul 2>nul
        )
