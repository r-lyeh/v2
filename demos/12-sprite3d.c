...
void game(unsigned frame) {
    ...
    if( ui_panel("Help", 0) ) {
        ui_label2("Freecam", UI_ICON(MOUSE) " RMB");
        ui_label2("Captain", UI_ICON(KEYBOARD) " Cursor keys");
        ui_label2("Crab", UI_ICON(KEYBOARD) " I/K/J/L keys");
        ui_label2("Shark", UI_ICON(KEYBOARD) " N/M");
        ui_panel_end();
    }

    if( ui_panel("Sprites", UI_OPEN)) {
        obj_edit(s1);
        obj_edit(s2);
        obj_edit(s3);
        obj_edit(s4);
...
int main() {
    ...
    for( app_create(0.75, 0); app_swap(); ) {
